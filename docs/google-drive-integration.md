...
## Blocage de facturation Google Cloud

Le code `OR_BACR2_44` apparaît lors de la création ou de l’activation du compte de facturation Google Cloud. Il ne signifie pas que le compte Google Drive est déconnecté. La documentation Google indique qu’un compte Cloud Billing en libre-service doit disposer d’un moyen de paiement valide, et que les moyens disponibles dépendent du pays et de la devise. Google distingue le profil de paiement, le compte de paiement et le compte Cloud Billing ; une erreur peut donc venir du profil Google Payments ou de l’éligibilité du moyen de paiement, pas de l’API Drive elle-même.

Pour l’import Drive, la voie la moins intrusive est de créer un projet Google Cloud séparé, activer uniquement Google Drive API, configurer Google Auth Platform et créer un client OAuth Web. Si Google impose néanmoins la facturation dans ce projet, il faut utiliser l’assistance Google Cloud ou un administrateur de facturation ; aucune donnée bancaire ne doit être transmise dans la conversation. Les pages officielles consultées sont : https://docs.cloud.google.com/billing/docs/how-to/resolve-issues, https://docs.cloud.google.com/billing/docs/how-to/payment-methods, https://developers.google.com/workspace/drive/api/quickstart/nodejs et https://developers.google.com/identity/protocols/oauth2/web-server.
