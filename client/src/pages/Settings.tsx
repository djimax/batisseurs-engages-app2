...
export default function Settings() {
  ...
  const { data: myPrivacyRequests = [], refetch: refetchPrivacyRequests } = trpc.auth.listMyPrivacyRequests.useQuery();
  const requestPrivacyRequest = trpc.auth.requestPrivacyRequest.useMutation({ onSuccess: () => { void refetchPrivacyRequests(); toast.success("Demande RGPD enregistrée"); } });
  const cancelPrivacyRequest = trpc.auth.cancelPrivacyRequest.useMutation({ onSuccess: () => { void refetchPrivacyRequests(); toast.success("Demande RGPD annulée"); } });
              ...
              <CardContent className="space-y-4">
                ...
                <div className="rounded-xl border border-primary/20 bg-primary/[0.03] p-4">
                  ...
                  <div className="mt-3 rounded-lg border bg-background/70 p-3">
                    <p className="text-sm font-medium">Demandes RGPD suivies</p>
                    <p className="mt-1 text-xs text-muted-foreground">Les demandes d’accès et d’export sont examinées par un administrateur. L’export direct ci-dessous reste disponible pour vos données authentifiées.</p>
                    <div className="mt-3 flex flex-wrap gap-2">
                      {(["access", "export", "erasure"] as const).map((requestType) => <Button key={requestType} type="button" variant="outline" size="sm" disabled={requestPrivacyRequest.isPending} onClick={() => requestPrivacyRequest.mutate({ requestType })}>{requestType === "access" ? "Demander l’accès" : requestType === "export" ? "Demander un export" : "Demander l’effacement"}</Button>)}
                    </div>
                    {myPrivacyRequests.length > 0 && <div className="mt-3 space-y-2" aria-live="polite">{myPrivacyRequests.slice(0, 5).map((request) => <div key={request.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md bg-muted/50 p-2 text-xs"><span>{request.requestType === "access" ? "Accès" : request.requestType === "export" ? "Export" : "Effacement"} · {request.status === "submitted" ? "soumise" : request.status === "in_review" ? "en examen" : request.status === "approved" ? "approuvée" : request.status === "rejected" ? "refusée" : request.status === "completed" ? "terminée" : "annulée"}</span>{request.status === "submitted" && <Button type="button" variant="ghost" size="sm" className="h-6 text-destructive" disabled={cancelPrivacyRequest.isPending} onClick={() => cancelPrivacyRequest.mutate({ id: request.id })}>Annuler</Button>}</div>)}</div>}
                  </div>
