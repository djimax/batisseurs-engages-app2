...
import { exportRowsToCSV } from "@/lib/exportLists";
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-6">
      ...
      {annualReportLoading ? <Card className="animate-pulse border-primary/10"><CardContent className="h-48 p-4" /></Card> : annualReport ? <Card className="border-primary/15 bg-gradient-to-br from-card via-card to-accent/5 animate-fade-in-up"><CardHeader><div className="flex flex-wrap items-start justify-between gap-3"><div><CardTitle>Rapport annuel {annualReport.year}</CardTitle><CardDescription>Membres, projets, événements et flux financiers issus des données enregistrées.</CardDescription></div><Button variant="outline" size="sm" onClick={() => exportRowsToCSV([{ indicateur: "Membres total", valeur: annualReport.members.total }, { indicateur: "Membres actifs", valeur: annualReport.members.active }, { indicateur: "Projets créés", valeur: annualReport.projects.total }, { indicateur: "Projets terminés", valeur: annualReport.projects.completed }, { indicateur: "Événements", valeur: annualReport.events.total }, { indicateur: "Cotisations EUR", valeur: annualReport.finance.cotisations.EUR }, { indicateur: "Cotisations XOF", valeur: annualReport.finance.cotisations.XOF }, { indicateur: "Dons EUR", valeur: annualReport.finance.donations.EUR }, { indicateur: "Dons XOF", valeur: annualReport.finance.donations.XOF }, { indicateur: "Dépenses EUR", valeur: annualReport.finance.expenses.EUR }, { indicateur: "Dépenses XOF", valeur: annualReport.finance.expenses.XOF }], [{ header: "Indicateur", value: (row) => row.indicateur }, { header: "Valeur", value: (row) => row.valeur }], `rapport_associatif_${annualReport.year}.csv`)}><Download className="mr-2 h-4 w-4" />Exporter CSV</Button></div></CardHeader><CardContent className="grid gap-3 sm:grid-cols-3"><div className="rounded-xl border bg-background/60 p-3"><p className="text-xs text-muted-foreground">Membres</p><p className="text-2xl font-semibold">{annualReport.members.total}</p><p className="text-xs text-muted-foreground">{annualReport.members.active} actifs</p></div><div className="rounded-xl border bg-background/60 p-3"><p className="text-xs text-muted-foreground">Projets / événements</p><p className="text-2xl font-semibold">{annualReport.projects.total} / {annualReport.events.total}</p><p className="text-xs text-muted-foreground">{annualReport.projects.completed} projets terminés</p></div><div className="rounded-xl border bg-background/60 p-3"><p className="text-xs text-muted-foreground">Solde</p><p className="text-2xl font-semibold">{annualReport.finance.balance.XOF.toLocaleString("fr-FR")} F CFA</p><p className="text-xs text-muted-foreground">{annualReport.finance.balance.EUR.toLocaleString("fr-FR", { maximumFractionDigits: 2 })} € · devises séparées</p></div></CardContent></Card> : null}
