...
export const assemblies = mysqlTable("assemblies", {
	...
	minutes: text(),
	attendanceCertificationStatus: mysqlEnum(['uncertified','certified']).default('uncertified').notNull(),
	attendanceCertifiedAt: timestamp({ mode: 'string' }),
	attendanceCertifiedBy: int(),
	attendanceProofHash: varchar({ length: 128 }),
...
export const events = mysqlTable("events", {
	...
	capacity: int(),
	attendanceToken: varchar({ length: 128 }),
	attendanceTokenExpiresAt: timestamp({ mode: 'string' }),
	attendanceTokenRevoked: int().default(0).notNull(),
	image: text(),
	createdBy: int().notNull(),
	createdAt: timestamp({ mode: 'string' }).default('CURRENT_TIMESTAMP').notNull(),
	updatedAt: timestamp({ mode: 'string' }).defaultNow().onUpdateNow().notNull(),
...
export const eventRegistrations = mysqlTable("event_registrations", {
	...
	eventId: int().notNull(),
	memberId: int().notNull(),
	status: mysqlEnum(['registered','waitlisted','attended','cancelled']).default('registered').notNull(),
